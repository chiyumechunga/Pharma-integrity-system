...
export default function PublicVerification() {
    ...
    const { data: scanResult, isPending, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            // STEP 3: Map the Spring Boot DTOs and handle Regulatory Recalls
            const sysStatus = provenanceData.productDetails?.currentStatus || 'UNKNOWN';
            let finalStatus = provenanceData.verificationStatus || (verificationData.isValid ? 'AUTHENTIC' : 'WARNING');

            // Force the top-level UI status to match the strict regulatory freeze
            if (sysStatus === 'RECALLED') {
                finalStatus = 'RECALLED';
            } else if (sysStatus === 'EXPIRED') {
                finalStatus = 'EXPIRED';
            }

            return {
                verificationStatus: finalStatus,
                message: verificationData.message || (finalStatus === 'RECALLED' ? 'CRITICAL: This product is under an active regulatory recall. Do not consume.' : ''),
                productDetails: {
                    ...
                    currentStatus: sysStatus,
                    serialNumber: provenanceData.productDetails.serialNumber
                },
                provenanceTimeline: provenanceData.provenanceTimeline || []
            };
        },
        enabled: !!scannedHash,
        retry: false
    ...
    return (
        <div className={styles.container}>
            ...
            {/* State 5: Success / Provenance / RECALL View */}
            {scanResult && !isError && !showReportForm && (
                <div className={styles.resultCard}>

                    {/* Safely guarded Status Header */}
                    <div className={`${styles.statusHeader} ${getStatusStyle(scanResult.verificationStatus)}`} style={{ textAlign: 'center', marginBottom: '24px' }}>
                        <span className="material-symbols-outlined" style={{ fontSize: '48px', marginBottom: '8px', color: scanResult.verificationStatus === 'RECALLED' ? '#d62828' : 'inherit' }}>
                            {scanResult.verificationStatus === 'AUTHENTIC' ? 'verified_user' :
                                (scanResult.verificationStatus === 'RECALLED' || scanResult.verificationStatus === 'COUNTERFEIT') ? 'warning' : 'gpp_maybe'}
                        </span>
                        <h2 style={{ color: scanResult.verificationStatus === 'RECALLED' ? '#d62828' : 'inherit' }}>
                            {scanResult.verificationStatus === 'AUTHENTIC' ? 'Product Confirmed Authentic' :
                                scanResult.verificationStatus === 'RECALLED' ? 'CRITICAL: PRODUCT RECALLED' :
                                    scanResult.verificationStatus === 'COUNTERFEIT' ? 'CRITICAL: SUSPECTED COUNTERFEIT' :
                                        'Verification Warning'}
                        </h2>
                        <p style={{ color: scanResult.verificationStatus === 'RECALLED' ? '#d62828' : 'inherit', fontWeight: 'bold' }}>
                            {scanResult.message}
                        </p>
                    </div>
